...
export default function SaleEditModal({
  ...
  const handleAddPayment = () => {
    ...
    onRegisterPayment(sale.id, amount, newPayment.method, newPayment.reference).then(result => {
      ...
      // Rollback on error
      console.error(err);
      alert("Error al registrar el abono: " + (err.response?.data?.message || err.message));
      ...
      footer={
        <>
          ...
          <Button 
            ...
            className="px-8 font-medium bg-primary hover:bg-primary-dark disabled:bg-slate-400 disabled:hover:bg-slate-400 disabled:cursor-not-allowed disabled:opacity-60"
